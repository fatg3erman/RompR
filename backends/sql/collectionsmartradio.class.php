...
class collectionSmartRadio {

	public static function preparePlaylist() {
		generic_sql_query('UPDATE Tracktable SET usedInPlaylist = 0 WHERE usedInPlaylist = 1');
		init_random_albums();
	}

	public static function doPlaylist($playlist, $limit) {
		...
		switch($playlist) {
			...
			default:
				if (preg_match('/^(\w+)\+(.+)$/', $playlist, $matches)) {
					...
					list($sqlstring, $tags) = self::$fn($matches[2]);
				} else {
		...
		$uris = self::getAllURIs($sqlstring, $limit, $tags, $random);
		$json = array();
		foreach ($uris as $u) {
	...
	private static function smart_radio_tag($param) {
	...
	private static function smart_radio_genre($param) {
	...
	private static function smart_radio_artist($param) {
	...
	private static function smart_radio_custom($param) {
	...
	private static function getAllURIs($sqlstring, $limit, $tags, $random = true) {
	...
	private static function getAveragePlays() {

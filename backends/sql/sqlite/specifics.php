...
function connect_to_database() {
	...
	try {
		...
		generic_sql_query('PRAGMA journal_mode=DELETE');
		generic_sql_query('PRAGMA cache_size=-4000');
		generic_sql_query('PRAGMA synchronous=OFF');
		generic_sql_query('PRAGMA threads=4');
	} catch (Exception $e) {
...
function check_sql_tables() {
	...
	if (generic_sql_query("CREATE TABLE IF NOT EXISTS Albumtable(".
		...
		"Domain CHAR(32), ".
		"Image VARCHAR(255), ".
		"justUpdated TINYINT(1) DEFAULT 0)"))
	{
		...
		"Image VARCHAR(255))"))
	{
		...
		if (generic_sql_query("CREATE INDEX IF NOT EXISTS ui ON RadioStationtable (PlaylistUrl)")) {
		} else {
			$err = $mysqlc->errorInfo()[2];
		...
		"PrettyStream TEXT)"))
	{
		...
		if (generic_sql_query("CREATE INDEX IF NOT EXISTS uri ON RadioTracktable (TrackUri)")) {
		} else {
			$err = $mysqlc->errorInfo()[2];
	...
	while ($sv < ROMPR_SCHEMA_VERSION) {
		switch ($sv) {
			...
			case 21:
				...
				generic_sql_query("ALTER TABLE Playcounttable ADD COLUMN LastPlayed TIMESTAMP DEFAULT NULL");
			...
			case 22:
				...
				generic_sql_query("ALTER TABLE Podcasttable ADD COLUMN Version TINYINT(2)");
				generic_sql_query("ALTER TABLE PodcastTracktable ADD COLUMN Guid VARCHAR(2000)");
				generic_sql_query("ALTER TABLE PodcastTracktable ADD COLUMN Localfilename VARCHAR(255)");
			...
			case 23:
				...
				generic_sql_query("DROP TRIGGER IF EXISTS updatetime");
			...
			case 25:
				...
				generic_sql_query("ALTER TABLE Tracktable ADD justAdded TINYINT(1) DEFAULT 1");
			...
			case 26:
				...
				generic_sql_query("ALTER TABLE Albumtable ADD justUpdated TINYINT(1) DEFAULT 1");
...
function delete_oldtracks() {
	...
	// generic_sql_query("CREATE TEMPORARY TABLE OldTracks AS SELECT TTindex FROM Tracktable JOIN Playcounttable USING (TTindex) WHERE Hidden = 1 AND DATETIME('now', '-6 MONTH') > DateAdded AND Playcount < 2");
	// generic_sql_query("DELETE FROM Tracktable WHERE TTindex IN (SELECT TTindex FROM OldTracks)");
}

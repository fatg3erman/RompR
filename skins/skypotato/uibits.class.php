...
class uibits {
	public static function albumTrack($data) {
		...
		if ($data['playable'] == 1 or $data['playable'] == 3) {
			...
			print '<div class="unplayable clicktrack ninesix containerbox" name="'.rawurlencode($data['uri']).'">';
		} else if ($data['uri'] == null) {
			print '<div class="playable '.$class.' ninesix draggable containerbox" name="'.$data['ttid'].'">';
		} else {
			print '<div class="playable '.$class.' ninesix draggable containerbox" name="'.rawurlencode($data['uri']).'">';
		}
